	...
	public function requestplaygrounds($sportid)
	{
	   try{
            ...
            foreach($areas as $area){
			    ...
			    $ar['matchcount']=$area->getFieldsMatchesCountAttribute();
				$ar['latitude']=$area->latitude;
				$ar['longitude']=$area->longitude;
				$ar['emails']=$area->emails;
				$ar['address']=$area->address;
				$ar['phones']=$area->phones;
                array_push($ps,$ar);
	   ...
	   $response=array();
	    ...
	    $matches = DB::table('fields')
			...
			foreach($matches as $match){
			  ...
			  $mt['status']=$match->status;
			  if(strtotime($mt['date'].' '.$mt['end_time'])>=strtotime(date('Y-m-d H:i'))){
			  		array_push($ms,$mt);
			  }
		...
		try{
			...
			if ($match->users->count() < $match->number_of_players) {
				...
				if(!empty($result)){				    
					...
					event(new JoinedToMatch($match, $userid));
				}else{
					$response['error']=true;
	...
	public function requestexitmatch($matchid,$userid)
	{
		try{
			...
			if($deletedRows>0){
				...
				event(new LeftMatch($match, $userid));
			}else{
				$response['error']=true;
				$response['message']="Player failed";
			}
	...
	*/
	public function requestmatchcreate(Request $request)
	{
		...
		$match->status =0;
			if($match->save()){
				// Notify All Users
				event(new CreatedNewMatch($match, "sportslug"));
				$response['error']=false;
	...
	*/
	public function postMessageAjax(Request $request)
    {
		try{
			...
			if ($request->user_id == $match->user_id OR in_array($request->user_id, $match->users->lists('id')->toArray())) {
